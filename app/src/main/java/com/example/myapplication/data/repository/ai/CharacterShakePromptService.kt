...
internal class CharacterShakePromptService(
    ...
    private fun buildCharacterShakePrompt(filters: CharacterShakeFilters): String = buildString {
        ...
        appendLine("- 星座偏好：${filters.zodiacSign.ifBlank { "不限，请随机" }}")
        appendLine("- MBTI：${filters.mbti.ifBlank { "不限，请随机" }}")
        appendLine()
        appendLine("【创作要求】")
        ...
        appendLine("4. 如果指定了星座或 MBTI，需要自然融入角色的行为倾向、关系节奏或标签，不要只做表面罗列。")
        appendLine("5. greeting 要像第一条消息，能直接出现在聊天里。")
        appendLine("6. example_dialogues 写 2 到 4 条，每条使用“用户：...\\n角色：...”格式。")
        appendLine("7. tags 写 3 到 6 个短标签；icon_name 必须从下列值中选一个：")
