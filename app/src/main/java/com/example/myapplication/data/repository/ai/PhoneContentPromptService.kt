...
internal class PhoneContentPromptService(
    ...
    suspend fun generateMomentCommentReplies(
        ...
        val prompt = buildString {
            ...
            appendLine("8. 不得把自己写成动态发布者正在想念、等待或邀约的人；不要生成“我也想你”“只许想我”“等我过去”等角色对角色恋爱式回复。")
            appendLine("9. 如果已有评论里有违背上述关系锚点的内容，不要模仿或延续。")
            appendLine("10. 严禁输出“默认角色”“角色”“NPC”“朋友A”等占位名。")
            appendLine("11. 严格输出 JSON 数组：[{\"author_type\":\"assistant|npc\",\"author_id\":\"...\",\"author_name\":\"...\",\"text\":\"...\"}]")
        }
        val content = core.requestCompletionContent(
            baseUrl = baseUrl,
            apiKey = apiKey,
