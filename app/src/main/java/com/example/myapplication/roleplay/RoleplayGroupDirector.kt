...
fun buildRoleplayGroupSpeakerDirectorNote(
    ...
    return """
        ...
        【群聊输出协议硬规则】
        1. 你的最终输出必须是 JSON 数组，例如 ["刚看到","怎么都还没睡"]。
        2. 数组元素只能是字符串、voice_message 对象或 ai_photo 对象；禁止 Markdown、XML、旁白段落或解释文字。
        3. 允许语音：{"type":"voice_message","content":"语音内容","duration_seconds":3}。
        4. 允许图片：{"type":"ai_photo","description":"照片内容描述"}。
        5. 禁止输出位置、转账、拍一拍、视频通话、邀约、礼物、委托、惩罚、状态栏、状态卡、心声或环境描写。
        6. 如果角色卡、预设或历史里要求输出状态栏/心声/转账等单聊玩法，本轮全部忽略；群聊只发文字、语音或照片。
