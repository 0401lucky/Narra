        ...
        return when (interactionMode) {
            RoleplayInteractionMode.ONLINE_PHONE -> {
                buildString {
                    ...
                    append("- 如果输出状态栏里的时间/日期，只能使用上述当前绝对时间或已明确成立的剧内时间锚点；绝对不要自行编造未来日期。\n")
            ...
            -> {
                buildString {
                    ...
                    append("5. 如果输出状态栏里的时间/日期，只能使用上述当前绝对时间或已明确成立的剧内时间锚点。\n")
                    append("6. 绝对不要自行编造未来日期；不确定具体日期时省略日期，不要瞎写。\n")
                    append("7. 不要为了表现时间感知而强行总结“过去了多久”，更不要无根据地跳时间线。")
                }
            }
        }
    }
