...
internal fun ChatScreenOverlays(
    ...
    if (showProfileSheet) {
        ProfileEditorSheet(
            ...
            maskCount = uiState.settings.normalizedUserPersonaMasks().size,
            defaultMaskName = uiState.settings.resolvedDefaultUserPersonaMask()?.name.orEmpty(),
            masks = uiState.settings.normalizedUserPersonaMasks(),
            defaultMaskId = uiState.settings.resolvedDefaultUserPersonaMask()?.id.orEmpty(),
